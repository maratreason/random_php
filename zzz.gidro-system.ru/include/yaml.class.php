...
// LOADING FUNCTIONS
  ...
  private function loadFromSource ($input) {
    $key = md5($input);
    // добавим кеширование файлов
    if(isset(yaml::$tmp[$key])){
        $input = yaml::$tmp[$key];
    } else {
...
// Enable use of y from command line
// The syntax is the following: php Spyc.php spyc.yaml

do {
  if (PHP_SAPI != 'cli') break;
  if (empty ($_SERVER['argc']) || $_SERVER['argc'] < 2) break;
  if (empty ($_SERVER['PHP_SELF']) || FALSE === strpos ($_SERVER['PHP_SELF'], 'yaml.class.php') ) break;
  $file = $argv[1];
