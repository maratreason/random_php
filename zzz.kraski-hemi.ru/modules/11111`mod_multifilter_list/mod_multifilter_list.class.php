...
class mod_multifilter_list extends Module{
    ...
    public function main($param=0){
        ...
        if(!self::$ajax){
            ...
            //$filter_full = func::getUrlFilterParams($prefix, $_POST['url']);
						//$filter_full = func::getAjaxFilterParams($prefix, $_POST);
						$filter_full = $_POST;
						//print_r($_POST);
        }
                ...
                if($params['type']=="range"){
                        ...
                        if($val1<=0||$val1==""){
                            ...
                            $seo_title[] = $params['title_start']."от ".$val1." до ".$val2.$params['title_end'];
														...
														if($params['h1_show']) $seo_h[] = $params['h1_start']." ".$val1."-".$val2.$params['h1_end'];
                        }
                }
        ...
        if(mod_multifilter_list::$ajax){
						//print_r($filter_full);
            echo '{"count":"'.$rescount.'", "url":"'.func::getFilterUrl($_POST).'"}';
            return 0;
        }
        ...
        while($item = $items->assoc()){
            /*// получим название каждой колонки и значение переведенное из базы в нормальный вид
            $data[$nom] = func::unitParamName($item);
            // также добавим в массив все поля страницы
            $data[$nom]['_col'] = $item;
            // отметим поля участвующие в фильтре
            foreach($item as $col=>$d){
                if(isset($areas[$col])){
                    $data[$nom]['_filter'][$col] = true;
                    //Engine::console($col." = ".$data[$nom]['_filter'][$col]);
                }
            }
        ...
        if(Engine::$urlAlias==""&&Engine::$page['title']!==""){} else {
            if($filter_url!=$action) Engine::$page['title'] = preg_replace("/\s{2,}/",' ',$filtercode['title_start']." ".implode(" ",$seo_title)."".$filtercode['title_end']);
            if($filter_url!=$action) Engine::$page['description'] = preg_replace("/\s{2,}/",' ',$filtercode['desc_start']." ".implode(" ",$seo_desc)."".$filtercode['desc_end']);
            if($filter_url!=$action) Engine::$page['content_title'] = preg_replace("/\s{2,}/",' ',$filtercode['h1_start']." ".implode(", ",$seo_h)."".$filtercode['h1_end']);
        }
        if($page_nomer>1) {
